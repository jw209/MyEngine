...
private:
    ...
    static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
        auto app = reinterpret_cast<MyEngine*>(glfwGetWindowUserPointer(window));
    ...
    /***** My functions *****/
    void compileShaders() {
        const char* batFilePath = "compile.bat";
    ...
    void deleteCompiledShaders() {
        ...
        if (std::remove(file1_path) == 0 && std::remove(file2_path) != 0) {
            throw std::runtime_error("failed to delete shaders");
        }
    }
    ...
    void generateGeometry(std::vector<Vertex>* vertices, std::vector<uint16_t>* indices, int gridSize) {
        ...
        while (isGeneratingVertices) {
            float x = -1.0f + (xIndex * step);
            float y = -1.0f + (yIndex * step);
            float z = 0.0f;
        ...
        xIndex = 0;
        yIndex = 0;
        while (isGeneratingIndices) {
            indices->push_back(xIndex + (yIndex * gridSize));
            indices->push_back(xIndex + (yIndex * gridSize) + 1);
            indices->push_back(xIndex + (yIndex * gridSize) + gridSize + 2);
            indices->push_back(xIndex + (yIndex * gridSize) + gridSize + 2);
            indices->push_back(xIndex + (yIndex * gridSize) + gridSize + 1);
            indices->push_back(xIndex + (yIndex * gridSize));
